...
- 构建产物为 `target/senhub-app-1.0.1.jar`（pom.xml 已使用 SenHub 品牌 artifactId：senhub-app）
...
**文件名**：`SenHub-app-1.0.1.tar.gz`

**包含内容**：

| 路径 | 说明 |
|------|------|
| `target/senhub-app-1.0.1.jar` | 主程序 JAR（Maven 构建产物） |
...
tar -czf /tmp/SenHub-app-1.0.1.tar.gz \
  target/senhub-app-1.0.1.jar \
...
- 应用包：`/tmp/SenHub-app-1.0.1.tar.gz`
