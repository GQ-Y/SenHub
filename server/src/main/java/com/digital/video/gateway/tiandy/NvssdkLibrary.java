...
public interface NvssdkLibrary extends Library {
    ...
    // 参数说明：
    // - _iLogonID: 登录句柄
    // - _iChannelNo: 通道号（0-based）
    // - _iActionType: 动作类型（PROTOCOL_MOVE_UP等）
    // - _iParam1: 参数1（左右移动时为速度）
    // - _iParam2: 参数2（上下移动时为速度）
    ...
    interface RAWFRAME_NOTIFY extends com.sun.jna.Callback {
        void apply(int uiID, Pointer pcData, int iLen, TiandySDKStructure.RAWFRAME_INFO ptRawFrameInfo,
                Pointer lpUserData);
    }
